...
/* Disciplina: MC458 -  2s/2014 - Laboratório 2 */

/* Gerador de grafos aleatórios - Lucas Carvalho */
...
for(i=0;i<n-1;i++){
	...
	for(j=i+1;j<n && j!=i;j++){ // A aresta (i,j) é guardada na posicao vetor_i[k] e vetor_j[k]
		vetor_i[k]=i;
		vetor_j[k]=j;
		k++;
	}
...
for(k=0;k<ng;k++){ // fornece ng grafos
    ...
    for (i=0;i<max;i++) {    	
	    j=rand()%max;       // j deve receber um inteiro aleatório do conjunto de i a w-1.
