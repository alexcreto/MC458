...
uint A2(uint n, vector<vector<uint> > M, vector<uint> grau, uint *seq_vert_ci, uint tempo_maximo, uint ordem[]){// A2 do enunciado
    /*
   * n 			=> 	Numero de vertices
   * M 			=> 	Matriz de adjacencia
  ...
  for(uint k = 0; k < n; k++){
    ...
    if(grau[k] == 0){
      MN = excluiVertice(M, k, &grau);
      G0 = A2(n-1, M0, grau, seq_vert_ci, tempo_maximo, proxOrdem) + 1;
    }
    //caso grau seja 1, ele tb estara na resposta
    else if(grau[k] == 1){
      M3 = excluiAdjacentes(M, k, &grau);
      G1 = A2(n-2, M1, grau, seq_vert_ci, tempo_maximo, proxOrdem) + 1;
    }
    //de resto continua td igual
      ...
      M3 = excluiAdjacentes(M, k, &modGrau);
      G3 = A2(n-1-adj, M3, grau, seq_vert_ci, tempo_maximo, proxOrdem) + 1;
    }
  }
