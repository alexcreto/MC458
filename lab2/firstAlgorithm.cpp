...
vector<vector<uint> > insertion_sort(vector<vector<uint> > M, int n) {
	uint i, j , a[n];
	for (i = 1; i < n; i++) {
		j = i;
...
uint A1(uint n, vector<vector<uint> > M, uint *tam_ci, uint *seq_vert_ci, uint tempo_maximo){
	/*
	...
	//Chamar recursao M0
	G0 = A1(n-1, M0, tam_ci, seq_vert_ci, tempo_maximo);
	
	
	// M1: v esta na solucao
	vector<vector<uint> >M1(n-1, vector<uint>(n-1,0));	
	M1 = M;
	
	uint adj = 0;				// Numero de vertices adjacentes a v
	for(uint i = 0; i < n; i++) adj += M1[0][i];
	M1 = insertion_sort(M1, n);	// Mexe na matriz de forma que 
								// os adj se acumulam no fundo
	
	//Chamar recursao M1
	G1 = A1(n-adj, M1, tam_ci, seq_vert_ci, tempo_maximo);
	
	if (G0 > G1) return G0;
	else return G1;
}

uint A2(uint n, vector<vector<uint> > M, uint *tam_ci, uint *seq_vert_ci, uint tempo_maximo); 	// A2 do enunciado
uint A3(uint n, vector<vector<uint> > M, uint *tam_ci, uint *seq_vert_ci, uint tempo_maximo);	// Algoritmo que devemos bolar que supere A1 e A2
...
int main (){
  ...
  while(nmrDeGrafos--){
	...
	cout<<A1(nmrDeVertices, graph, 0,0,0);
  return 0;
}
